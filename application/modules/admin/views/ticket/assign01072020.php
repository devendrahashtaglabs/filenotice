                    ...
                    <div class="card-body floating-laebls">
								...
								<div class="form-group inputBox focus">
									...
									foreach ($consultant['data'] as $key => $value) {
										...
										if(!empty($categorywiseconsultant)){
											$catarray 		= explode(',',$categorywiseconsultant->category_id);
											$subcatarray	= explode(',',$categorywiseconsultant->subcategory_id);
											if( in_array($ticket->categoryid,$catarray) && in_array($ticket->subcategory_id,$subcatarray) && $value->step5_verify_time != '0000-00-00 00:00:00'){
												if(!empty($mapticketdata)){
													if($value->user_id != $mapticketdata->consultant_id){
														$option[$value->usersId] = ucfirst($value->account_type) .' - '.ucfirst($value->name).' - '.$value->email;
													}
												}else{
													$option[$value->usersId] = ucfirst($value->account_type) .' - '.ucfirst($value->name).' - '.$value->email;
												}
						...
						<div class="row">
							<?php
								...
								echo '&nbsp;&nbsp;<a href="' . base_url($this->session->userdata('admins')['user_type'].'/ticket/ticket_list') . '" class="btn btn-default">Cancel</a>';
                            ?>
						</div>
                    </div>
					<?php echo form_close(); ?>
