        ...
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                ...
                <li class="nav-item has-treeview <?php echo (in_array($uri_segment, array('ticket')) ? $menu_open : ''); ?>">
                    ...
                    <ul class="nav nav-treeview">
                       <li class="nav-item">
                            <a href="<?php echo base_url($userType . '/ticket/servicelist/?status=5'); ?>" class="nav-link <?php echo((in_array(end($this->uri->segments), array('servicelist', 'edit', 'create')) || ($segment2 == 'ticket' && $segment3 == 'edit')) ? $li_class : ''); ?>">
								<i class="fa fa-leanpub nav-icon"></i>
                                <p>Leads Ticket List</p>
                            </a>
                        </li>
						<li class="nav-item">
                            <a href="<?php echo base_url($userType . '/ticket/servicelist'); ?>" class="nav-link <?php echo((in_array(end($this->uri->segments), array('servicelist', 'edit', 'create')) || ($segment2 == 'ticket' && $segment3 == 'edit')) ? $li_class : ''); ?>">
                                <i class="fa fa-ticket nav-icon"></i>
                                <p>All Service Request List</p>
                            </a>
                        </li>
						<li class="nav-item">
                            <a href="<?php echo base_url($userType . '/ticket/servicelist/?status=10'); ?>" class="nav-link <?php echo((in_array(end($this->uri->segments), array('edit', 'create')) || ($segment2 == 'ticket' && $segment3 == 'edit')) ? $li_class : ''); ?>">
                                <i class="fa fa-ticket nav-icon"></i>
                                <p>New Request List</p>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="<?php echo base_url($userType . '/ticket/servicelist/?status=20'); ?>" class="nav-link <?php echo(((end($this->uri->segments) == 'assign_list') || (in_array($segment3, array('assign', 'conversation')))) ? $li_class : ''); ?>">
								<i class="fa fa-handshake-o nav-icon"></i>
								<p>Under Process</p>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="<?php echo base_url($userType . '/ticket/servicelist/?status=90'); ?>" class="nav-link <?php echo(((end($this->uri->segments) == 'completed_list') || (in_array($segment3, array('completed_list')))) ? $li_class : ''); ?>">
