...
class User_model extends CI_Model {
	...
	public function getUserDataById($id, $userType) {
		$this->db->cache_on();
        $this->db->select('nw_user_tbl.id as user_tbl_id,nw_user_tbl.*,details.*,nw_countrys_tbl.name as countryName, nw_states_tbl.name as stateName, nw_cities_tbl.city_name as cityName');
        $this->db->where('nw_user_tbl.id',$id);
        $this->db->from('nw_user_tbl');
	...
	public function getagentbycategory($id,$category,$subcategory,$city=null,$state=null) 
	{
        $this->db->cache_on();
		$this->db->select('*');
	...
	public function getAgentdetailByTicket($ticketid,$consultant_id) 
	{
        $this->db->cache_on();
		$this->db->select('DISTINCT(agent.name),agent.mobile,users.email');
        $this->db->from('nw_agent_tbl as agent');
		$this->db->join('nw_agent_ticket_map_tbl as agent_map', 'agent_map.agent_id = agent.user_id','left');
		$this->db->join('nw_ticket_map_tbl as ticket_map', 'agent_map.consultant_id = ticket_map.consultant_id','left');
		$this->db->join('nw_user_tbl as users', 'users.id = agent.user_id','left');
        $this->db->where('agent_map.ticket_id', $ticketid);
        $this->db->where('agent_map.consultant_id', $consultant_id);
		$allowedstatus = array(20,21,22,92,93);
        $this->db->where_in('agent_map.ticket_status', $allowedstatus);
	...
	public function getAgentUserList($userType=null,$column=null,$start=null,$limit=null,$orderBy=null, $type="DESC",$statue=null,$consultant_id=null){
        $data['count'] = 0;
        $this->db->select('users.*,users.status AS usersStatus,users.id as usersId, details.*,IF(details.name<>"",details.name,"'.DEFAULT_VALUE.'") As name,details.id as detail_id,details.status AS userStaus,,nw_countrys_tbl.name as countryName, nw_states_tbl.name as stateName',FALSE);
             // $this->db->select('users.*,users.status AS usersStatus,users.id as usersId, details.*,IF(details.name<>"",details.name,"'.DEFAULT_VALUE.'") As name,details.id as detail_id,details.status AS userStaus,,nw_countrys_tbl.name as countryName, nw_states_tbl.name as stateName,nw_ticket_map_tbl.status as ticket_status',FALSE);
             // count(nw_ticket_map_tbl.status) AS ticket_status
		...
		if(!empty($consultant_id)){
            $this->db->where('details.consultant_id', $consultant_id,false);
        }
        $this->db->from( USERTABLE .' AS users');
        $this->db->join('nw_agent_tbl as details', 'users.id=details.user_id', 'left');
        $this->db->join('nw_countrys_tbl', 'details.country_id=nw_countrys_tbl.id', 'left');
