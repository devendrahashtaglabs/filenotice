...
class ticket_model extends CI_Model {
    ...
    public function getTicketById($id) {
		$this->db->cache_on();
        $this->db->select('nw_ticket_tbl.*,nw_ticket_tbl.id as ticketid,nw_ticket_tbl.ticket_id as custom_id,nw_customer_tbl.name as customername,nw_customer_tbl.sname as customersname,nw_customer_tbl.title as title,nw_category_tbl.name as category_name, IF(nw_category_tbl.parent_id="0",nw_category_tbl.id,nw_category_tbl.parent_id) as categoryid, nw_user_tbl.id as userid,nw_user_tbl.email as useremail,nw_ticket_map_tbl.consultant_id,nw_ticket_map_tbl.assign_date', false);
	...
	public function getagenttabledata($key,$value,$table,$column='*') {
		$this->db->cache_on();
	...
	public function updateAgentRecordsByticketid( $ticketid, $consultantid, $data, $table) {
		$this->db->where('ticket_id', $ticketid);
	...
	public function getcustomerfeedback($ticket_id,$customer_id) {
		$this->db->select('*');
	...
	public function createticketonpay($data) {
		$this->db->insert('nw_ticket_tbl', $data);			
		$insert_id = $this->db->insert_id();
        return $insert_id;
    }
	public function userticketdata($user_id){
		$this->db->select('*');
		$this->db->from('nw_ticket_tbl');
	...
	public function getTickets($userId,$limit=null,$orderBy=null,$orderType=null,$ticket_status=null,$status=null) {
		$this->db->cache_on();
        $data['count'] = 0;
        $this->db->select('tickets.*,tickets.ticket_id as customId,maps.id as mapid, maps.ticket_id, maps.consultant_id,'
