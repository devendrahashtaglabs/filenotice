    ...
    <div class="sidebar">
        ...
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                ...
                <li class="nav-item has-treeview <?php echo (in_array($this->uri->segment(1), array('ticket')) && !in_array($this->uri->segment(2), array('feedback')) ? $menu_open : ''); ?>">
                    ...
                    <ul class="nav nav-treeview">
                        <?php if ($userType == 3 || $userType == 4) { ?>
                            <li class="nav-item">
                                <a href="<?php echo base_url('ticket/servicelist/?status=20'); ?>" class="nav-link <?php echo((in_array($uri_segment, array('assign', 'conversation'))) ? $li_class : ''); ?>">
									<i class="fa fa-handshake-o nav-icon"></i>
                                    <p>Under Process</p>
                                </a>
                            </li>
							<li class="nav-item">
                                <a href="<?php echo base_url('ticket/servicelist/?status=90'); ?>" class="nav-link <?php echo(((end($this->uri->segments) == 'completed')) ? $li_class : ''); ?>">
                        ...
                        <?php } else { ?>
							...
							<li class="nav-item">
                                <a href="<?php echo base_url('ticket/servicelist/?status=10'); ?>" class="nav-link <?php echo((in_array($uri_segment, array('new_assign_list', 'edit'))) ? $li_class : ''); ?>">
				...
				<?php 
					if ($userType == 3){ 
						if($userDetails->account_type != 'freelancer'){
				?>
