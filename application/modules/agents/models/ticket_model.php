...
class ticket_model extends CI_Model {
    ...
    public function getTicketById($id) {
		$this->db->cache_on();
        $this->db->select('nw_ticket_tbl.*,nw_ticket_tbl.id as ticketid,nw_ticket_tbl.ticket_id as custom_id,nw_customer_tbl.name as customername,nw_category_tbl.name as category_name, IF(nw_category_tbl.parent_id="0",nw_category_tbl.id,nw_category_tbl.parent_id) as categoryid, nw_user_tbl.id as userid,nw_ticket_map_tbl.consultant_id,nw_ticket_map_tbl.assign_date', false);
	...
	public function checkticketassigntoagent($ticket_id,$consultant_id,$assign_agent_status = null,$ticket_status = null,$status = null) {
		$this->db->select('maps.*');
        $this->db->from('nw_ticket_map_tbl As maps');
		$this->db->where('maps.ticket_id',$ticket_id);
		$this->db->where('maps.consultant_id',$consultant_id);
		if(!empty($assign_agent_status)){
			$this->db->where('maps.assign_agent_status',$assign_agent_status);
		}else{
			$this->db->where('maps.assign_agent_status','10');
		}
		if(!empty($ticket_status)){
			$this->db->where('maps.ticket_status',$ticket_status);
		}else{
			$this->db->where('maps.ticket_status','20');
		}
		if(!empty($status)){
			$this->db->where('maps.status',$status);
		}else{
    ...
    public function getConsultantTicketListCount($userId,$limit=null,$orderBy=null,$orderType=null,$ticket_status=null,$status=null,$type=null){ 
        ...
        $this->db->select('tickets.*,tickets.ticket_id as customId,maps.id as mapid, maps.ticket_id, maps.consultant_id,'
                . 'users.email AS customer_email,categorys.name as category_name,cusers.email AS consultant_email,consultant.user_id,consultant.name AS consultant_name,consultant.mobile AS consultant_mobile,consultant.photo AS consultant_photo,IF(maps.id<>"","true","false") As mapstatus', false);
        $this->db->from(TICKETTABLE.' AS tickets');
        $this->db->join('nw_agent_ticket_map_tbl As maps', 'tickets.id = maps.ticket_id', 'left');
        $this->db->join('nw_category_tbl As categorys', 'tickets.category_id=categorys.id', 'left');
        $this->db->join('nw_user_tbl As users', 'tickets.customer_id = users.id', 'left');
        $this->db->join('nw_consultant_tbl As consultant', 'maps.consultant_id = consultant.user_id', 'left');
        $this->db->join('nw_user_tbl As cusers', 'consultant.user_id = cusers.id', 'left');
        ...
        if($userId!='' && $this->session->userdata('agents')['user_type'] == 'agent'){
            $this->db->where('maps.agent_id',$userId);
        }
        if($type == 'new'){
	...
	public function getallnewnotification($userid) {
        ...
        $this->db->where(array('conversation.msg_to'=>$userid,'conversation.read_status'=>'0','tickets.status'=>'1'));
		 $this->db->where_not_in('tickets.ticket_status',array(10,90,91));
	...
	public function checkticketisassigntoagent($userid,$consultant_id) {
		$this->db->cache_on();
		$this->db->select('*');
        $this->db->from('nw_agent_ticket_map_tbl As agent_map');
		$this->db->join('nw_ticket_tbl As tickets', 'tickets.id = agent_map.ticket_id', 'left');
		$this->db->join('nw_conversation_tbl As conversation', 'agent_map.ticket_id = conversation.ticket_id', 'left');
        $this->db->where(array('agent_map.agent_id'=>$userid,'conversation.msg_to'=>$consultant_id,'tickets.status'=>'1'));
		$this->db->where_not_in('tickets.ticket_status',array(10,90,91));
        $data['data'] = $this->db->get()->result();
		if(!empty($data['data'])){
            $data['count'] = count($data['data']);
	...
	public function getticketremarkstatus( $where_in_data ) {
		$this->db->select('*');
		$this->db->from('nw_master_ticket_status_tbl');
		$this->db->where_in('status_code',$where_in_data);
		$this->db->order_by('status', 'asc');
		$query  = $this->db->get()->result();
        return $query;
    }
	public function getallticketdata($ticke_id) {
		$this->db->cache_on();
		$this->db->select('tickets.*,maps.assign_agent_status as agent_assign_status,maps.ticket_status as maps_ticket_status,maps.status as maps_status,agent_ticket.ticket_status as agent_ticket_ticket_status,agent_ticket.status as agent_ticket_status');
        $this->db->from('nw_ticket_map_tbl As maps');
        $this->db->join('nw_ticket_tbl As tickets', 'tickets.id = maps.ticket_id', 'left');
		$this->db->join('nw_agent_ticket_map_tbl As agent_ticket', 'tickets.id = agent_ticket.ticket_id', 'left');	
		$this->db->where('maps.ticket_id',$ticke_id);
		$this->db->where('tickets.status','1');
		$data = $this->db->get()->row();
        return $data;
	}
	public function updateTicketsTable($key,$value,$table,$data){
